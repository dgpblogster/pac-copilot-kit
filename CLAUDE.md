...
| `docs/pac-copilot-kit-design.md` | **Design** — full architecture, guardrail funnel, MCP verb set, repo layout, open questions, and the **decision log** with rationale (§bottom) | A design decision is made or revised, or a §12 open question closes |
...
5. **One Web API funnel.** Every Dataverse Web API call routes through `Invoke-PckDataverseRequest`. No `Invoke-RestMethod` in public cmdlets. All error translators for the pac war stories live in that one helper.
6. **Guardrail-authoring contract.** *(amended 2026-08-16: terminology; "guard" became "guardrail" everywhere, with the two tiers named preflight checks and error translators)* New guardrail = one file under `Private/Guardrails/`, one Pester test proving the untranslated call fails and the translated call succeeds, one line in `docs/war-stories.md`. No guardrails merge without all three.
...
9. **Distinct exit code range for preflight vs operational failure.** `10..19` = preflight check tripped (environment misconfigured). Everything else = operation failed. Keeps CI diagnostics unambiguous.
