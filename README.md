...
**NOTE:** do not trust `pac`'s exit codes in your own scripts, because the MSI-installed `pac` on your PATH is a launcher that discards them. The same failing command returns 0 through `pac.cmd` and 1 through the versioned `pac.exe` underneath it; the launcher shim is dated September 2019 and the problem is known upstream as [#912](https://github.com/microsoft/powerplatform-build-tools/issues/912) and [#1027](https://github.com/microsoft/powerplatform-build-tools/issues/1027), both open. The kit compensates: its `pac` funnel refuses to believe a zero exit code that arrives with an `Error:` line, so the kit's own exit codes stay honest whichever way `pac` was installed. If you script `pac` directly, install it as a dotnet tool, which skips the launcher, or check the output yourself. War story 7 has the full isolation.

## How it earned its scars

The kit was not tested politely. Every guardrail gets an adversarial pass before it ships: attack tests written red-first, expected to expose something, and the first such pass landed seven real defects out of thirteen attempts, including four ways to spell the same forbidden request past the flagship check. The fix was structural (every request is normalized before any check sees it), and the practice stuck.

Then the live runs kept the honesty going. The very first live pipeline run was refused by the profile-drift check, because the author's own machine had a drifted `pac` profile: the founding war story, firing on the person who wrote it down. And the kit's integration tests caught the war-stories inventory itself overclaiming: the savedquery failure that was documented as universal turns out to reproduce on most Quick Find views while a minority accept the same call, and the discriminator is still unidentified after ruling out custom-versus-system, the search sync flag, and table type. If you figure out what separates them, [the war-stories doc](docs/war-stories.md) is where that hunt lives, and credit awaits.
