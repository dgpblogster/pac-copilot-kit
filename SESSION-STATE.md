...
- **Guardrail funnel:** two tiers — preflight in `Private/Assert-*.ps1`, wrap-and-translate in single `Invoke-PckDataverseRequest`. New guardrail = file + Pester test + war-stories doc line. (CLAUDE canons 5, 6, 9; design §6.)
...
- **v0.1 scope:** `Invoke-PckCopilotPipeline` + `Export-PckSolutionBackup` + war-stories guardrails + MCP shim from day one. (Design §12.1.)
...
**Done 2026-08-15, first live run (owner's tenant, Workbench environment):** everything passed first try except the part that was supposed to fail, and that part taught the most. Auth chain, discovery, WhoAmI, harness classification: green live, both harnesses present. The savedquery integration test then caught war story 1 overclaiming: the fetchxml PATCH succeeds on a minority of views (`privilegecheckerrun`, virtual `aaduser`) and fails with `0x80040216` on most (custom and system, standard and elastic, synced or not). Discriminator unknown; sync flag, custom/system, and TableType ruled out by experiment. Guardrail redesigned to attempt-and-translate (`Convert-PckSavedQueryFetchXmlError`). Also verified live: `bot.authenticationmode` (war story 3), enabling `Assert-PckAgentAuthMode` and the auth-mode fields on `Get-PckAgentInfo`. War story 4 (returnedtypecode filter quirk) recorded as inventory. Suite: 68 passing including 3 live.

**Done 2026-08-15, walking skeleton:** `Connect-PckPowerPlatform`, `Invoke-PckDataverseRequest`, `Get-PckAgentInfo`, `Assert-PckAgentHarness`, `Assert-PckSavedQueryFetchXmlRoute`, token chain per §12.8, `docs/war-stories.md` stories 1 and 2. Then an adversarial pass, red first: 13 attack tests, 7 real defects (4 guardrail bypasses by respelling the request, empty-set `-Json` shape, plain-http URL acceptance), all fixed. 44 unit tests passing; 3 integration tests tagged `Integration` await a live run. **Working practice: every guardrail gets an adversarial bypass hunt, red first, before it ships.**
...
**Done 2026-08-16, live pipeline run:** end to end with the owner participating. The drift guardrail refused the genuinely misaligned profile first (exit 16, the founding war story caught live), then with an aligned `PckWorkbench` profile the full loop ran clean: lint over 15 Microsoft-authored scaffold files with zero false positives, pack to the predicted zip path, import and publish into the live environment. Probe agent and solution deleted afterward; owner's working profile restored; a `PckWorkbench` pac profile (index 5) was left behind deliberately for future kit work. War story 7 came out of the cleanup: pac exits 0 on errors it prints, now guarded in the funnel with red-first tests. Suite: 106 unit + 4 live.

**Done 2026-08-16, MCP server:** `pac-copilot-kit-mcp` shipped and proven. Prior-art check first: Microsoft's `pac-mcp` (newer CLI line) exposes pac commands as tools and rides the active pac profile, so it inherits both the capability gap and the drift risk; ours exposes the guarded lifecycle, complement not duplicate, recorded in the design log. Seven verbs over stdio, startup preflight, injection-safe env-var bridge with a cmdlet allowlist, typed exit codes surfaced to the client, and `explain-failure` reasoning locally over the session's last failure. `Sync-PckCopilotAgent` shipped as the cmdlet behind `pull-agent`. Proof: stdio smoke test, plus a live full-stack probe in which the drift guardrail refused through MCP (exit 16) and `explain-failure` explained it unprompted. Samples for all three clients in `samples/mcp-config/`. Suite: 109 unit + 4 live + 2 MCP scripts.
...
4. Write the first Pester test for a war-story guardrail: the `savedquery` PATCH `0x80040216` case. Locks the guardrail-authoring contract (canon 6) with a real example before any second guardrail exists.
