...
1. **MCP shim** (`src/pac-copilot-kit-mcp/`): Node 20, stdio, seven verbs shelling to the module. The last v0.1 component. **First step: prior-art check on `pac copilot mcp`**, a subcommand discovered on 2026-08-16 ("information about local MCP server"); if Microsoft ships an official Copilot Studio MCP surface, the kit's shim must position against it deliberately rather than duplicate it (trust-but-verify).
