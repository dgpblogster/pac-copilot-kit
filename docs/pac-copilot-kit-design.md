...
**Public home:** https://github.com/dgpblogster/pac-copilot-kit *(public; v0.1.0 released 2026-08-16)*
**License:** MIT
...
| 2026-08-16 | Release sequencing decided: everything waits for the two source articles. No package publishes while the repo is private, because Gallery and npm packages expose the code publicly and carry links that would 404, which is inconsistent with the private-until-articles rationale. Launch day is one motion: articles live, repo public, Gallery publish, npm publish, tag v0.1.0. Both packages are prepped and validated (Publish-Module WhatIf clean; npm pack clean with README and LICENSE) | User chose "wait for the articles" over publish-now and flip-now |
| 2026-08-16 | War story 7 root cause isolated (owner, then reproduced on the dev machine): the MSI layout's `pac.launcher.exe` (v1.0.7, September 2019) discards exit codes while the versioned `pac.exe` returns them correctly. Upstream as powerplatform-build-tools #912 and #1027, both open; the isolation was posted to #912. Workaround for raw scripting: dotnet-tool install skips the launcher. The kit's funnel guardrail is unchanged and covers both install flavors. README, war-stories, and ci/README updated with the finding | Owner's post-10 draft investigation, verified live |
| 2026-08-16 | v0.1.0 released, by the owner, ahead of the articles: `PacCopilotKit` on the Gallery and `pac-copilot-kit-mcp` on npm under The Workbench Blog accounts, superseding the same-day wait-for-articles sequencing. Gallery artifact verified byte-identical to HEAD. Post-publish sweep completed and `v0.1.0` tagged | Owner published; sweep this session |
| 2026-08-16 | Repo flipped public the same day, also by the owner, completing the launch: repo, release page, Gallery, and npm all live. The front-running protection was the owner's to waive and they waived it; the articles now follow the release rather than precede it | Owner |
